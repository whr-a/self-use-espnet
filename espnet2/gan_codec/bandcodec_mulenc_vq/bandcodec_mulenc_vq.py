...
import bigvgan
class Bandcodec_mulenc_vq(AbsNonGANCodec):
    """DAC model."""
    ...
    def __init__(
        ...
        self.generator = Bandcodec_mulenc_vqGenerator(**generator_params)
...
class Bandcodec_mulenc_vqGenerator(nn.Module):
    ...
    def __init__(
        ...
        if preload == True:
            ...
            prefix = "codec.generator.encoders."
            enc_dict = {}
            for k, v in state_dict.items():
                if k.startswith(prefix):
                    new_k = k[len(prefix):]
                    enc_dict[new_k] = v
            incompatible = self.encoders.load_state_dict(enc_dict, strict=False)
